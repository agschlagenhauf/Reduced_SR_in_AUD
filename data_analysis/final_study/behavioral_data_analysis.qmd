...
```{r}
logreg_test_path_condition_df_full <- df %>%
  filter(phase == "test",
         state == 1) %>%
  mutate(correct_path = as.factor(correct_path),
         condition = as.factor(condition),
...
logreg_test_path_condition_df_lowrisk_control <- df %>%
  filter(phase == "test",
         state == 1,
         audit_group == "low-risk",
         version == "control") %>%
  mutate(correct_path = as.factor(correct_path),
         condition = as.factor(condition),
         version = as.factor(version)) %>%
...
logreg_test_path_condition_df_alcohol <- df %>%
  filter(phase == "test",
         state == 1,
         version == "alcohol") %>%
  mutate(correct_path = as.factor(correct_path),
...
logreg_test_path_condition_df_lowrisk <- df %>%
  filter(phase == "test",
         state == 1,
         audit_group == "low-risk") %>%
  mutate(correct_path = as.factor(correct_path),
...
#### Model 1: Low-risk drinkers and non-monetary context, compare against transition revaluation

```{r}
contrasts(logreg_test_path_condition_df_lowrisk_control$condition) <- contr.treatment(5, base = 5)

model1 <- glmer(correct_path ~ condition + (1 | ID), data = logreg_test_path_condition_df_lowrisk_control, family = "binomial")
tab_model(model1, transform = NULL, show.est = T, show.stat = T, auto.label = FALSE, CSS = list(css.table = '+font-size: 14;'))
```

#### Model 2: Low-risk drinkers and non-monetary context, compare against policy revaluation

```{r}
contrasts(logreg_test_path_condition_df_lowrisk_control$condition) <- contr.treatment(5, base = 3)

model2 <- glmer(correct_path ~ condition + (1 | ID), data = logreg_test_path_condition_df_lowrisk_control, family = "binomial")
tab_model(model2, transform = NULL, show.est = T, show.stat = T, auto.label = FALSE, CSS = list(css.table = '+font-size: 14;'))
```

#### Model 3a: Alcohol dataset, compare against transition revaluation

```{r}
contrasts(logreg_test_path_condition_df_alcohol$audit_group) <- contr.treatment(2, base = 1)
contrasts(logreg_test_path_condition_df_alcohol$condition) <- contr.treatment(5, base = 5)

model3a <- glmer(correct_path ~ condition*audit_sum + (1 | ID), data = logreg_test_path_condition_df_alcohol, family = "binomial", glmerControl(optCtrl=list(maxfun=1e5), optimizer = "bobyqa"))
tab_model(model3a, transform = NULL, show.est = T, show.stat = T, auto.label = FALSE, CSS = list(css.table = '+font-size: 14;'))
```

#### Model 3b: Low-risk dataset, compare against transition revaluation

```{r}
contrasts(logreg_test_path_condition_df_lowrisk$version) <- contr.treatment(2, base = 2)
contrasts(logreg_test_path_condition_df_lowrisk$condition) <- contr.treatment(5, base = 5)

model3b <- glmer(correct_path ~ condition*version + (1 | ID), data = logreg_test_path_condition_df_lowrisk, family = "binomial", glmerControl(optCtrl=list(maxfun=1e5), optimizer = "bobyqa"))
tab_model(model3b, transform = NULL, show.est = T, show.stat = T, auto.label = FALSE, CSS = list(css.table = '+font-size: 14;'))
```

#### Model 3: Full dataset, compare against transition revaluation

```{r}
contrasts(logreg_test_path_condition_df_full$version) <- contr.treatment(2, base = 2)
contrasts(logreg_test_path_condition_df_full$audit_group) <- contr.treatment(2, base = 1)
contrasts(logreg_test_path_condition_df_full$condition) <- contr.treatment(5, base = 5)

mm_version <- model.matrix(correct_path ~ 1 + version, data=logreg_test_path_condition_df_full)
version_alc <- mm_version[,2] # version alc compared to version monetary

mm_group <- model.matrix(correct_path ~ 1 + audit_group, data=logreg_test_path_condition_df_full)
group_harmful <- mm_group[,2] # group harmful compared to low-risk

mm_condition <- model.matrix(correct_path ~ 1 + condition, data=logreg_test_path_condition_df_full)
...
condition_reward <- mm_condition[,5] # condition policy compared to transition
...
#                   condition_control:group_harmful:version_alc +
#                   condition_goalstate:group_harmful:version_alc +
#                   condition_policy:group_harmful:version_alc + condition_reward:group_harmful:version_alc + 
#                   (1 | ID), data = logreg_test_path_condition_df_full, family = "binomial", glmerControl(optCtrl=list(maxfun=1e5), optimizer = "bobyqa"))

model3 <- glmer(correct_path ~ 
                  condition*audit_group*version + 
                  (1 | ID), data = logreg_test_path_condition_df_full, family = "binomial", glmerControl(optCtrl=list(maxfun=1e5), optimizer = "bobyqa"))
...
```{r}
contrasts(logreg_test_path_condition_df_alcohol$audit_group) <- contr.treatment(2, base = 1)
contrasts(logreg_test_path_condition_df_alcohol$condition) <- contr.treatment(5, base = 4)

model4 <- glmer(correct_path ~ condition*audit_sum + (1 | ID), data = logreg_test_path_condition_df_alcohol, family = "binomial")
...
contrasts(logreg_test_path_condition_df_full$version) <- contr.treatment(2, base = 2)
contrasts(logreg_test_path_condition_df_full$audit_group) <- contr.treatment(2, base = 1)
contrasts(logreg_test_path_condition_df_full$condition) <- contr.treatment(5, base = 4)

model5 <- glmer(correct_path ~ condition*version*audit_sum + (1 | ID), data = logreg_test_path_condition_df_full, family = "binomial", glmerControl(optCtrl=list(maxfun=1e5), optimizer = "bobyqa"))
...
## Correst paths across 5 test trials within participant

```{r}
test_correct_path_ID_component_df <- df %>%
  filter(phase == "test",
         state == 1) %>%
  group_by(version, ID, component) %>%
  summarise(n = n(),
...
test_correct_path_ID_component_df %>%
  kable("html") %>%
  kable_styling(font_size = 18,
                latex_options = "scale_down")
```
...
```{r}
# box plot
ggplot(test_correct_path_ID_component_df, aes(x=component, y=percent_correct)) +
  geom_boxplot(fill="lightblue") +
  geom_jitter(width = 0.1) +
  scale_y_continuous(labels = scales::comma) +
  facet_wrap(~ version) +
