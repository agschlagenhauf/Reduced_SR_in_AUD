...
# Load packages
packages <- c("ggplot2", "dplyr", "tidyr", "kableExtra", "gmodels", "lme4", "sjPlot", "viridis")
...
A group of participants eraneously answered the question "To how much money does this path lead?" in the alcohol version of the task. These are excluded for all further analyses.
...
audit_df <- trial_df %>%
  select(ID, group, version, audit_pre, audit_post, audit_mean) %>%
  distinct()
...
contrasts(logreg_learning_paths_group_version_df$version) <- contr.treatment(2, base = 2)
contrasts(logreg_learning_paths_group_version_df$group) <- contr.treatment(2, base = 2)
...
Does % correct paths starting from state 1 change over the course if the learning phase within one condition, and does it do so in different ways per group, version, and condition?
...
```{r}
# bar plot
ggplot(learning_paths_group_version_environment_df, aes(x=environment, y=mean_correct_paths*100)) +
  geom_bar(stat="identity", aes(fill=environment)) +
  geom_errorbar(aes(ymin=ci_l*100, ymax=ci_u*100), width=.2, position=position_dodge(.9)) +
  scale_y_continuous(limits = c(0, 105)) +
...
```{r}
logreg_learning_paths_group_version_env_df <- trial_df %>%
  filter(phase == "learning",
         state == 1) %>%
  mutate(correct_path = as.factor(correct_path),
         group = as.factor(group),
...
contrasts(logreg_learning_paths_group_version_env_df$version) <- contr.treatment(2, base = 2)
contrasts(logreg_learning_paths_group_version_env_df$group) <- contr.treatment(2, base = 2)
contrasts(logreg_learning_paths_group_version_env_df$environment) <- contr.sum(10)

log_model_learning_paths_group_version_env <- glmer(correct_path ~ group*version*environment + (1 | ID), data = logreg_learning_paths_group_version_env_df, family = "binomial", control=glmerControl(optimizer="bobyqa"))
tab_model(log_model_learning_paths_group_version_env, transform = NULL, show.est = T, show.stat = T, auto.label = FALSE, CSS = list(css.table = '+font-size: 14;'))
```
...
### Condition index per environment
