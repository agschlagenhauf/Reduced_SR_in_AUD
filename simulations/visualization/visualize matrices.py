...
df_sr = pd.read_csv(f"/Users/milenamusial/Library/CloudStorage/OneDrive-Charité-UniversitätsmedizinBerlin/PhD/04_B01/WP3/Reduced_SR_in_AUD/simulations/results/full_sr_nsimulations110_alpha0.9_beta0.9_gamma0.5.csv")

### Filter

df_sr = df_sr.loc[df_sr['condition'] == condition].reset_index(drop=True)

successful_learners = df_sr.loc[df_sr['phase'] == "learning_test"].reset_index(drop=True)

df_sr = df.groupby("simulation_number").apply(
    lambda g: (g.loc[g["phase"] == "learning", "check_column"] == check_value).any()
).reset_index(name="contains_value")

### Get raw matrices at the end of Learning
