...
##### define paths
data_path <- "WP3_DATA/PILOT_2/behavioral_data"

##### read in json data and convert to df
data_raw <- readLines(file.path(data_path, "jatos_results_data_20240620201941.txt"))
...
data_df <- data_df %>% 
  unnest(c(aggregate_results.trialResults, 
           ...
           aggregate_rating_results.ratingResults),
         names_sep = "_",
         keep_empty = TRUE)
         
##############################  trial-df   ##############################  
trial_df <- data_df %>%
  mutate(rating_results.rating = coalesce(rating_results.rating, aggregate_rating_results.rating),
         rating_results.ratingResults_state = coalesce(rating_results.ratingResults_state, aggregate_rating_results.ratingResults_state),
         rating_results.ratingResults_value = coalesce(rating_results.ratingResults_value, aggregate_rating_results.ratingResults_value),
         rating_results.ratingResults_RT = coalesce(rating_results.ratingResults_RT, aggregate_rating_results.ratingResults_RT)
         ) %>%
  ...
  mutate(environment = case_when(((variation %in% c("A1", "B1", "C1", "D1", "E1") & condition_index == 1) |
                                    (variation %in% c("A2", "B2", "C2", "D2", "E2") & condition_index == 5) |
                                    (variation %in% c("A3", "B3", "C3", "D3", "E3") & condition_index == 4) |
                                    (variation %in% c("A4", "B4", "C4", "D4", "E4") & condition_index == 3) |
                                    (variation %in% c("A5", "B5", "C5", "D5", "E5") & condition_index == 2)) ~ "white_modern",
                                 ((variation %in% c("A1", "B1", "C1", "D1", "E1") & condition_index == 2) |
                                    (variation %in% c("A2", "B2", "C2", "D2", "E2") & condition_index == 1) |
                                    (variation %in% c("A3", "B3", "C3", "D3", "E3") & condition_index == 5) |
                                    (variation %in% c("A4", "B4", "C4", "D4", "E4") & condition_index == 4) |
                                    (variation %in% c("A5", "B5", "C5", "D5", "E5") & condition_index == 3)) ~ "blue_floral",
                                 ((variation %in% c("A1", "B1", "C1", "D1", "E1") & condition_index == 3) |
                                    (variation %in% c("A2", "B2", "C2", "D2", "E2") & condition_index == 2) |
                                    (variation %in% c("A3", "B3", "C3", "D3", "E3") & condition_index == 1) |
                                    (variation %in% c("A4", "B4", "C4", "D4", "E4") & condition_index == 5) |
                                    (variation %in% c("A5", "B5", "C5", "D5", "E5") & condition_index == 4)) ~ "messy_green",
                                 ((variation %in% c("A1", "B1", "C1", "D1", "E1") & condition_index == 4) |
                                    (variation %in% c("A2", "B2", "C2", "D2", "E2") & condition_index == 3) |
                                    (variation %in% c("A3", "B3", "C3", "D3", "E3") & condition_index == 2) |
                                    (variation %in% c("A4", "B4", "C4", "D4", "E4") & condition_index == 1) |
                                    (variation %in% c("A5", "B5", "C5", "D5", "E5") & condition_index == 5)) ~ "orange_tile",
                                 ((variation %in% c("A1", "B1", "C1", "D1", "E1") & condition_index == 5) |
                                    (variation %in% c("A2", "B2", "C2", "D2", "E2") & condition_index == 4) |
                                    (variation %in% c("A3", "B3", "C3", "D3", "E3") & condition_index == 3) |
                                    (variation %in% c("A4", "B4", "C4", "D4", "E4") & condition_index == 2) |
                                    (variation %in% c("A5", "B5", "C5", "D5", "E5") & condition_index == 1)) ~ "red_brown")) %>%
  arrange(ID,
          component,
          trial,
          rating_no)
...
rating_df <- rating_df %>%
  ...
  mutate(rating_diff_state1 = case_when((rating_state == "1LeftRating" & correct_first_state_action == "left") ~ (rating_value - lead(rating_value)),
         ...
         rating_diff_state2 = case_when((rating_state == "2LeftRating" & correct_second_state_action == "left") ~ (rating_value - lead(rating_value)),
                                        (rating_state == "2RightRating" & correct_second_state_action == "right") ~ (rating_value - lag(rating_value))),
         rating_diff_state3 = case_when((rating_state == "3LeftRating" & correct_third_state_action == "left") ~ (rating_value - lead(rating_value)),
                                        (rating_state == "3RightRating" & correct_third_state_action == "right") ~ (rating_value - lag(rating_value)))) %>%
  mutate(rating_diff = coalesce(rating_diff_state1, rating_diff_state2, rating_diff_state3))
