...
```{r}
predictor <- "uppsp_total"

if (predictor == "audit_sum_mean") {
  
  trial_df <- trial_df %>%
    mutate(predictor = audit_sum_mean,
           group_new = group)
  ...
  trial_df <- trial_df %>%
    mutate(predictor = casa_gf_unaware,
           group_new = case_when(predictor >= median(predictor) ~ "High scores",
                               predictor < median(predictor) ~ "Low scores"))
...
trial_df <- trial_df %>%
  mutate(group_new = case_when(predictor >= median(predictor) ~ "High scores",
                               predictor < median(predictor) ~ "Low scores"))
```
