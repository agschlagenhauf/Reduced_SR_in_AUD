...
dummies <- demo_psych %>%
  ...
  select(group_binary, version_binary)
...
p.adjust(p.values, method = "holm", n = length(p.values))
...
oci_plot <- ggplot(correlation_plot_df, aes(x = oci_total, y = sr_use)) +
    geom_point(aes(color = version)) +  # Color by version, shape by group
    geom_smooth(aes(color = version, fill=version), method = 'lm', alpha = 0.2, size=1.3) +  # Color by version, linetype by group
    scale_x_continuous("UPPSP score", limits = c(0, 65)) +
    scale_y_continuous("Individual SR use", limits = c(0, 13)) +
    scale_color_viridis(option = "turbo", discrete = TRUE, name = "Version") +  # Color legend for version
    scale_fill_viridis(option = "turbo", discrete = TRUE) +  # Color legend for version
    theme_light(base_size = 12) +
...
correlation_df_monetary <- demo_psych %>%
  ...
  filter(version == "Monetary version") %>%
  select(audit_sum_pre, aud_sum, drinking_days, drinks_per_day, binge_days, casa_gf_unaware, casa_gf_nonvolitional, uppsp_total, oci_total, redsr_use)
...
# extract individual slopes for condition (contrast reward vs. transition)
random_intercepts <- coef(model3_rs_vers)$ID[,"(Intercept)"]
...
# calculate individual adjusted slopes for condition (contrast reward vs. transition)
individual_mb_slopes <- fixed_effects["(Intercept)"] +
                    fixed_effects["groupHarmful drinkers"] * dummies$group_binary +  
                    fixed_effects["versionAlcohol version"] * dummies$version_binary +
                    fixed_effects["groupHarmful drinkers:versionAlcohol version"] * (dummies$group_binary * dummies$version_binary) + 
                    random_intercepts  # Add individual deviations
...
correlation_df_monetary <- demo_psych %>%
  ...
  filter(version == "Monetary version") %>%
  select(audit_sum_pre, aud_sum, drinking_days, drinks_per_day, binge_days, casa_gf_unaware, casa_gf_nonvolitional, uppsp_total, oci_total, mb_use)
...
binge_plot <- ggplot(correlation_plot_df, aes(x = binge_days, y = mb_use)) +
    ...
    geom_smooth(aes(color = version, fill=version), method = 'lm', alpha = 0.1, size=1.3) +  # Color by version, linetype by group
    scale_x_continuous("Binge days / past 3 months", limits = c(0, 90)) +
    scale_y_continuous("Individual MB use", limits = c(-11, -2)) +
    scale_color_viridis(option = "turbo", discrete = TRUE, name = "Version") +  # Color legend for version
    scale_fill_viridis(option = "turbo", discrete = TRUE) +  # Color legend for version
    theme_light(base_size = 12) +
    theme(legend.position = "bottom") +
...
# extract individual slopes for condition (contrast reward vs. transition)
random_intercepts <- coef(model5_rs_vers)$ID[,"(Intercept)"]
...
# calculate individual adjusted slopes for condition (contrast reward vs. transition)
individual_mf_slopes <- fixed_effects["(Intercept)"] +
                    fixed_effects["groupHarmful drinkers"] * dummies$group_binary +  
                    fixed_effects["versionAlcohol version"] * dummies$version_binary +
                    fixed_effects["groupHarmful drinkers:versionAlcohol version"] * (dummies$group_binary * dummies$version_binary) + 
                    random_intercepts  # Add individual deviations

# invert values, higher slopes represent less MF use
individual_mf_slopes <- individual_mf_slopes*-1
...
correlation_df_monetary <- demo_psych %>%
  ...
  filter(version == "Monetary version") %>%
  select(audit_sum_pre, aud_sum, drinking_days, drinks_per_day, binge_days, casa_gf_unaware, casa_gf_nonvolitional, uppsp_total, oci_total, mf_use)
...
binge_plot <- ggplot(correlation_plot_df, aes(x = binge_days, y = mf_use)) +
    ...
    geom_smooth(aes(color = version, fill=version), method = 'lm', alpha = 0.1, size=1.3) +  # Color by version, linetype by group
    scale_x_continuous("Binge days / past 3 months", limits = c(0, 90)) +
    scale_y_continuous("Individual MF use", limits = c(-5, 5)) +
    scale_color_viridis(option = "turbo", discrete = TRUE, name = "Version") +  # Color legend for version
    scale_fill_viridis(option = "turbo", discrete = TRUE) +  # Color legend for version
    theme_light(base_size = 12) +
    theme(legend.position = "bottom") +
...
#### Inspect relation control performance - psychometric measures

```{r}
model3_control_rs_vers <- readRDS(file.path(model_path, "model3_control_rs_vers.rds"))

# get fixed effects
fixed_effects <- fixef(model3_control_rs_vers)

# extract individual slopes for condition (contrast reward vs. transition)
random_slopes <- coef(model3_control_rs_vers)$ID[,"condition1"]
...
# calculate individual adjusted slopes for condition (contrast reward vs. transition)
individual_mf_slopes <- fixed_effects["groupHarmful drinkers:condition1"] * dummies$group_binary +  
                    fixed_effects["versionAlcohol version:condition1"] * dummies$version_binary +
                    fixed_effects["groupHarmful drinkers:versionAlcohol version:condition1"] * (dummies$group_binary * dummies$version_binary) + 
                    random_slopes  # Add individual deviations
...
correlation_plot_df <- demo_psych %>%
  arrange(prolific_ID) %>%
  mutate(control_performance = individual_control_slopes)

binge_plot <- ggplot(correlation_plot_df, aes(x = binge_days, y = control_performance)) +
    geom_point(aes(color = version)) +  # Color by version, shape by group
    geom_smooth(aes(color = version, fill=version), method = 'lm', alpha = 0.1, size=1.3) +  # Color by version, linetype by group
    scale_x_continuous("Binge days / past 3 months", limits = c(0, 90)) +
    scale_y_continuous("Individual control_performance", limits = c(3, 16)) +
    scale_color_viridis(option = "turbo", discrete = TRUE, name = "Version") +  # Color legend for version
    scale_fill_viridis(option = "turbo", discrete = TRUE) +  # Color legend for version
    theme_light(base_size = 12) +
    theme(legend.position = "bottom") +
...
audit_plot <- ggplot(correlation_plot_df, aes(x = audit_sum_pre, y = control_performance)) +
    geom_point(aes(color = version)) +  # Color by version, shape by group
    geom_smooth(aes(color = version, fill=version), method = 'lm', alpha = 0.1, size=1.3) +  # Color by version, linetype by group
    scale_x_continuous("AUDIT score", limits = c(0, 35)) +
    scale_y_continuous("Individual control_performance", limits = c(3, 16)) +
