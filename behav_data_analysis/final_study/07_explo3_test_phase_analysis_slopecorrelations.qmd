...
binge_plot <- ggplot(correlation_plot_df, aes(x = binge_days, y = sr_use)) +
    ...
    scale_y_continuous("(Reduced) \n random-policy SR use", limits = c(-3, 13)) +
...
ggsave(filename = "suppl_explo_slopes.png", path = figure_path, width = 14, height = 10, device='png', dpi=600)
```

#### Inspect relation random-policy SR use - psychometric measures
...
# calculate individual adjusted slopes for condition (contrast reward vs. transition)
individual_sr_slopes <- fixed_effects["condition4"] +  # Main effect of condition 1
                    fixed_effects["group1:condition4"] * dummies$group_binary +  # Adjust for group interaction
                    fixed_effects["version1:condition4"] * dummies$version_binary +  # Adjust for version interaction
                    fixed_effects["group1:version1:condition4"] * (dummies$group_binary * dummies$version_binary) +  # Three-way interaction
                    random_slopes  # Add individual deviations
...
binge_plot <- ggplot(correlation_plot_df, aes(x = binge_days, y = sr_use)) +
    ...
    geom_smooth(aes(color = version, fill=version), method = 'lm', alpha = 0.2, size=1.3) +  # Color by version, linetype by group
    scale_x_continuous("Binge days / past 3 months", limits = c(0, 90)) +
    scale_y_continuous("Individual SR use", limits = c(-3, 13)) +
    scale_color_manual(values = c(custom_colors[5], custom_colors[2])) +
    scale_fill_manual(values = c(custom_colors[5], custom_colors[2])) +
    theme_light(base_size = 12) +
    theme(legend.position = "bottom") +
    guides(
      color = guide_legend(order = 1, override.aes = list(size = 0)),  # Version legend comes first
...
```{r}
# sr correlations
r.values <- rcorr(as.matrix(cbind(slopes_df$individual_reward_slopes, slopes_df$individual_goalstate_slopes, slopes_df$individual_policy_slopes)), type="spearman")$r[1:3,1]
r.values
p.values <- rcorr(as.matrix(cbind(slopes_df$individual_reward_slopes, slopes_df$individual_goalstate_slopes, slopes_df$individual_policy_slopes)), type="spearman")$P[1:3,1]
p.adjust(p.values, method = "bonferroni", n = length(p.values))

# red sr correlations
r.values <- rcorr(as.matrix(cbind(individual_mb_slopes, individual_mf_slopes, individual_redsr_slopes, individual_sr_slopes)), type="spearman")$r[,"individual_redsr_slopes"]
r.values
p.values <- rcorr(as.matrix(cbind(individual_mb_slopes, individual_mf_slopes, individual_redsr_slopes, individual_sr_slopes)), type="spearman")$P[,"individual_redsr_slopes"]
p.adjust(p.values, method = "BH", n = 3)
```
